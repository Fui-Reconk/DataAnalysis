...
- 📂 **文件加载**：多选加载 xls/xlsx/**DBF**（DBF 自动识别编码、中文 GBK 兼容，可与其他格式混合匹配）；支持**多工作表**——Excel 含多个 Sheet 时可自由勾选要添加的表；文件添加过任意表后即进入**暂存池**，之后可随时直接添加其其它表，无需重新选择文件；**导入/重载时自动按 `[column_map]` 替换列名缩写**；列表行**右键菜单**支持预览前几行、打开文件、设为基准表、替换列名缩写、重载、删除等
...
- 🔍 **过滤筛选**：下拉框选择**列名**与**条件**（等于/不等于/大于/大于等于/小于/小于等于/包含/不包含/为空/不为空）并输入比较值；条件行可自由**增删**（默认一行），多行为「并且」关系；过滤完成后自动弹出结果预览（可在 `config.cfg` 关闭）；可**隐藏不需要的列**（勾选对话框，基准表列不可隐藏、始终显示；影响过滤结果预览与导出，原始合并结果不变；调整后自动弹预览）
...
3. **过滤筛选** → 每行选择列名、条件并输入比较值（可点「添加条件」增行）→ 「应用过滤」（可选步骤）
   - 条件或数值不合法时弹窗提示（含行号），程序不会崩溃；未执行匹配时提示先匹配
4. **统计与导出** → 点击「导出结果」
   - Sheet1：合并且过滤后的最终数据
