...
- 📂 **文件加载**：多选加载 xls/xlsx/**DBF**（DBF 自动识别编码、中文 GBK 兼容，可与其他格式混合匹配）；支持**多工作表**——Excel 含多个 Sheet 时可自由勾选要添加的表；文件添加过任意表后即进入**暂存池**，之后可随时直接添加其其它表，无需重新选择文件；列表行**右键菜单**支持预览前几行、打开文件、设为基准表、替换列名缩写、重载、删除等
- 🔗 **匹配配置**：主键下拉框只列**基准表**的列，选择唯一标识项后执行左连接；匹配完成后自动弹出结果预览（可在 `config.cfg` 关闭）
